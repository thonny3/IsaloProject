...
export default function TbaleApprovisionementRAmi() {
  ...
  const { setEdit, setOpen, ShowEdit, listApproRami } = useFournisseur();
          ...
          {currentData.map((row) => (
            <tr key={row.approvisionnement.id} className="text-gray-600 mt-2">
              <td className="px-4 py-2 flex items-center">
                {row.produits.map((product) => (
                  <span key={product.id} className="ml-2 ">
                    <span className="text-gray-800">{product.produit.nom}</span>[ {product.quantite}],{" "}
              ...
              <td className="px-4 py-2 flex space-x-2">
                <button className=" hover:text-blue-700 text-sm bg-yellow-300 p-1 rounded-md">
